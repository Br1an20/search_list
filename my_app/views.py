...
def newSearch(request):
    try:
        # Step 1: Get input and search in targeted website
        search = request.POST.get('search')
        models.Search.objects.create(search=search)
        final_urls = BASE_CRAIGSLIST_URL.format(quote_plus(search))
        reponse = requests.get(final_urls)
        data = reponse.text
        soup = BeautifulSoup(data, features='html.parser')
        # Step 2: Looking for info in the website
